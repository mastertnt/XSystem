            ...
            foreach (var lKey in msInheritedTypesCache.Keys)
            {
                ...
                if (lThis != null)
                {
                    ...
                    else
                    {
                        ...
                        var lFilteredTypes = lFoundTypes.Where(pType => pType.GetConstructors().Any(pConstructor => pConstructor.GetParameters().Count() == lConstructorArgCount));
                        msInheritedTypesCache[lKey].Concat(lFilteredTypes);
                    }
                }
            }
        }
